...
PRIVATE int rm_file(char* filename, struct inode* dir_inode);
PUBLIC void rm_dir(char* filename, struct inode* dir_inode);
/*****************************************************************************
 *                                do_unlink
 *****************************************************************************/
/**
...
{
	//printl("rmtest\n");
	//printl("invalid_inode:%d", INVALID_INODE);
	//char pathname[MAX_PATH];
	...
	int inode_nr = search_file_in_dir(filename, dir_inode);
	if (inode_nr == INVALID_INODE) {	/* file not found */
		//printl("FS::do_unlink():: search_file() returns "
			//"invalid inode: %s\n", pathname);
		return -1;
...
PUBLIC void rm_dir(char* filename, struct inode* dir_inode)
{
	...
	int inode_nr=search_file_in_dir(filename, dir_inode);
	struct inode* tar_inode=get_inode(dir_inode->i_dev, inode_nr);

	if(tar_inode->i_mode==I_REGULAR){
		rm_file(filename, dir_inode);
		//printl("%d\n", tar_inode->i_num);
		return;
	}
	
	int dir_blk0_nr = tar_inode->i_start_sect;
	int nr_dir_blks = (tar_inode->i_size + SECTOR_SIZE - 1) / SECTOR_SIZE;
	int nr_dir_entries =
	  tar_inode->i_size / DIR_ENTRY_SIZE; /**
	...
	for (i = 0; i < nr_dir_blks; i++) {
		RD_SECT(dir_inode->i_dev, dir_blk0_nr + i);
		void* tembuf[512];
		memcpy(tembuf, fsbuf, 512);
		pde = (struct dir_entry *)tembuf;
		for (j = 0; j < SECTOR_SIZE / DIR_ENTRY_SIZE; j++,pde++) {
			if (++m > nr_dir_entries)
				break;
			if (pde->inode_nr!=0){
				//printl("%s\n", pde->name);
				//char* tem;
				//memcpy(tem, pde->name, strlen(pde->name));
				rm_dir(pde->name, tar_inode);
			}
		}
